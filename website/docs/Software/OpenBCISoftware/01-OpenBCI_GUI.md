...
:::info MacOS Ganglion BLE Workaround
...
Then follow the instructions for [running from the IDE](01-OpenBCI_GUI.md/#running-the-openbci-gui-from-the-processing-ide).

Note that applications exported from Processing 3.5.4 do not appear to ask for Bluetooth permissions even if the JDK is updated before exporting. Processing 3.5.4 is no longer maintained so downstream fixes may exist in Processing 4.
:::
