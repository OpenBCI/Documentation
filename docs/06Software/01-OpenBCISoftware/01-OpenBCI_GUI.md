...
Users have the option to save data as a CSV text file or BDF+ format. **By default, the GUI stores all user data and raw EEG recordings in `Documents/OpenBCI_GUI` and names each session with an autogenerated timestamp by default.** Older versions of the GUI save data to `SavedData` in the same root directory as the app.
