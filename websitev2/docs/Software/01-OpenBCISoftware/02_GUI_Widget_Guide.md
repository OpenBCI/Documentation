...
For certain data types, it helps to change the `` to match the data type. Example: Focus is a single number output, so just set `` to 1. Here's a screenshot streaming 3 data types to Python:

![gui windows override high dpi scaling](../../assets/SoftwareImages/OpenBCISoftware/gui_lslStreaming_Example.png)

You can find the [example Python script here](https://github.com/OpenBCI/OpenBCI_GUI/tree/master/Networking-Test-Kit). Note: You will need to do `` if the dependency is not already installed.
...
-   To see how to access global data structures like FFT data and Time Series data, check out how the [other widgets](https://github.com/OpenBCI/OpenBCI_GUI/tree/master/OpenBCI_GUI) are accessing data in their `` functions
...
_Debugging_
If you've added any `` or `` in your widget, you can use the Console Log feature to see these messages, or just look at the console in your favorite IDE.

_Screen Resized_
Some objects may need to be resized or re-positioned when the user changes the GUI screen size. This is why the widget class has the `` method. Something not displaying right after the GUI is resized? This is where you need to add code. Typically, object x, y, width, and height are declared when the widget is instantiated, and again in `` using relative positioning.

_Settings_
To test that everything is working, you should see the data in a settings JSON file for each data mode in the `` folder. JSON files can be viewed as text. Confirm that your new JSON object and data are being written to file. Finally, confirm that the settings work properly when configuring the widget, saving, and loading.

_GUI Crash_
One of the most common ways to crash the GUI with a new widget is drawing `` data, or some other error related to ``. Another common crash is an ``, which will print to the console of Atom, VSCode, or Processing IDE.
